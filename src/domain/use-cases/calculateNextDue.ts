 ...
 */
export function calculateNextDue(rule: TaskRule): number {
  const baseTime = rule.lastDoneAt ?? Date.now();
 ...
 */
export function updateRuleAfterTaskCompletion(rule: TaskRule): TaskRule {
  const now = Date.now();
  return {
    ...rule,
    lastDoneAt: now,
